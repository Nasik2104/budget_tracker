...
class SpendingsDb(DefaultInterface):
    ...
    def get_spendings(self):
        self.cursor.execute("""
            SELECT id, name, category_id, spend_date, spendings, is_spending FROM spendings
        """)
        return self.cursor.fetchall()

    def get_spending(self, id):
        self.cursor.execute("""
            SELECT name, category_id, spend_date, spendings, is_spending FROM spendings WHERE id=?
        """, (id))
        return self.cursor.fetchone()
